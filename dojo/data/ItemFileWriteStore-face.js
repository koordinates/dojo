dojo.provide("dojo.data.ItemFileWriteStore-face");
dojo.required("dojo.data.ItemFileWriteStore");
...
/* dojo.data.api.Write */
		...
		for(var key in keywordArgs){
			if (dojo.isOwnProperty(keywordArgs, key)) {
				if(key === this._storeRefPropName || key === this._itemNumPropName){
					// The user is trying to do something like
	...
	revert: function(){
		...
		for(identity in this._pending._modifiedItems){
			if (dojo.isOwnProperty(this._pending._modifiedItems, identity)) {
		...
		for(identity in this._pending._deletedItems){
			if (dojo.isOwnProperty(this._pending._deletedItems, identity)) {
		...
		for(identity in this._pending._newItems){
			if (dojo.isOwnProperty(this._pending._newItems, identity)) {
