dojo.provide("dojox.widget.Toaster-face");
dojo.required("dojox.widget.Toaster");
...
dojo.declare("dojox.widget.Toaster", [dijit._Widget, dijit._Templated], {
		...
		setContent: function(/*String|Function*/message, /*String*/messageType, /*int?*/duration){
			...
			for (var type in this.messageTypes) {
				if (dojo.isOwnProperty(this.messageTypes, type)) {
		...
		_cancelHideTimer:function(){
			if (this._hideTimer){
				dojo._getWin().clearTimeout(this._hideTimer);
		...
		_setHideTimer:function(duration){
			...
			if(duration>0){
				this._cancelHideTimer();
				this._hideTimer = dojo._getWin().setTimeout(dojo.hitch(this, function(evt){
		...
		show: function(){
			...
			if (!this._scrollConnected) {
				this._scrollConnected = dojo.connect(dojo._getWin(), "onscroll", this, this._placeClip);
			}
		},
