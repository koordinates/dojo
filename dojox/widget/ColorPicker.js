...
dojo.required(['dijit._Templated', 'dijit.form._FormWidget'], function() {
	// NOTE: Lose the preserved dojo reference
	...
	dojo.declare("dojox.widget.ColorPicker",
		...
		postCreate: function(){
			...
			if(typeof this.colorUnderlay.style.filter == 'string'){ 
		...
		_setTimer: function(/* d.dnd.Mover */mover){
			...
			this._timer = dojo._getWin().setInterval(d.hitch(this, "_updateColor"), 45);	
		},
		
		_clearTimer: function(/* d.dnd.Mover */mover){
			dojo._getWin().clearInterval(this._timer);
			this._timer = null;
