...
dojo.required(['dojox.charting.plot2d.Base', 'dojox.lang.utils'], function() {
	...
	dojo.declare("dojox.charting.plot2d.Default", dojox.charting.plot2d.Base, {
		...
		render: function(dim, offsets){
			...
			var t = this.chart.theme, stroke, outline, color, marker, lpoly, ht, vt, events = this.events();
			var sh, spoly, run, frontMarkers, outlineMarkers, shadowMarkers, shadowStroke, shadowColor;

			// NOTE: Create these once and need better names for first two

			var createPolygons1 = function(v, i){
				return {
					x: ht(i + 1) + offsets.l,
			...
			var createPolygons2 = function(v, i){
				return {
					x: ht(v.x) + offsets.l,
			...
			var createShadowedPolygons = function(c){
				return {x: c.x + sh.dx, y: c.y + sh.dy};
			};

			var setFrontMarkers = function(c, i){
			...
			for(var i = this.series.length - 1; i >= 0; --i){
				...
				if(typeof run.data[0] == "number"){
					lpoly = dojo.map(run.data, createPolygons1, this);
				}else{
					lpoly = dojo.map(run.data, createPolygons2, this);
				}
				...
				frontMarkers = outlineMarkers = shadowMarkers = null;
				if(this.opt.shadows && stroke){
					sh = this.opt.shadows;
					shadowColor = new dojo.Color([0, 0, 0, 0.3]);
					spoly = dojo.map(lpoly, createShadowedPolygons);
					...
					if(this.opt.markers){
						shadowMarkers = dojo.map(spoly, setFrontMarkers, this);
					}
				}
				if(this.opt.lines){
					if(outline){
				...
				if(this.opt.markers){
					...
					dojo.forEach(lpoly, createPaths, this);
					if(events){
						dojo.forEach(frontMarkers, connectEvents, this);
					}
				}
				run.dirty = false;
			}
