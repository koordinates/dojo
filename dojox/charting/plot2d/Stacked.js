...
(function(){
	...
	dojo.declare("dojox.charting.plot2d.Stacked", dojox.charting.plot2d.Default, {
		...
		render: function(dim, offsets){
			var ht, i, j, lpoly, run, s, sh, shadowColor, shadowStroke, spoly, v, vt;
			var frontMarkers, outlineMarkers, shadowMarkers;
			var stroke, outline, color, marker;

			var createPolygons = function(v, i){
				return {
					x: ht(i + 1) + offsets.l,
			...
			for(i = this.series.length - 1; i >= 0; --i){
				...
				run.cleanGroup();
				s = run.group;
				lpoly = dojo.map(acc, createPolygons, this);
				...
				if(this.opt.shadows && stroke){
					...
					if(this.opt.markers){
						shadowMarkers = dojo.map(spoly, createPaths, this);
					}
				}
				if(this.opt.lines){
					if(outline){
				...
				if(this.opt.markers){
					...
					dojo.forEach(lpoly, setFrontMarkers, this);
					if(events){
