...
var addCommand = function(name, args){
	...
	// add in dt
	// is usually args[1] but there is an exception
...
var stop = function(){
	...
	if(c.length){
		...
		var scroll = {y: (window.pageYOffset || dojo.doc.documentElement.scrollTop || dojo.body().scrollTop || 0),
		x: (window.pageXOffset || (window.dojo?dojo._fixIeBiDiScrollLeft(dojo.doc.documentElement.scrollLeft):undefined) || dojo.body().scrollLeft || 0)};
		div.style.left = scroll.x+"px";
		div.style.top = scroll.y+"px";
		var h1 = dojo.doc.createElement('h1');
		h1.innerHTML = "Your code:";
		div.appendChild(h1);
		var pre = dojo.doc.createElement('pre');
...
var getSelector = function(node){
	...
	if(typeof node =="string"){
		...
		var nodes = dojo.doc.getElementsByTagName(node.nodeName);
		var i;
		...
		return "function(){ return dojo.doc.getElementsByTagName('" + node.nodeName+"')["+i+"]; }";
	}
};
...
var getMouseButtonObject = function(b){
	...
	// NOTE: Duplication (logic belongs in events module)
...
var onmousedown = function(e){
	...
	addCommand("doh.robot.mousePress",[getMouseButtonObject(e.button), 0]);
};
...
var onclick = function(e){
	...
	addCommand("doh.robot.mouseClick",[getMouseButtonObject(e.button), 0]);
};
...
var onmouseup = function(e){
	...
	addCommand("doh.robot.mouseRelease",[getMouseButtonObject(e.button), 0]);
};
...
dojo.connect(dojo.doc,"onmousedown",onmousedown);
dojo.connect(dojo.doc,"onmouseup",onmouseup);
dojo.connect(dojo.doc,"onclick",onclick);
dojo.connect(dojo.doc,"onkeypress",onkeypress);
dojo.connect(dojo.doc,"onkeyup",onkeyup);
dojo.connect(dojo.doc,"onmousemove",onmousemove);
dojo.connect(dojo.doc,"onmousewheel",onmousewheel);
dojo.connect(dojo.doc,'DOMMouseScroll',onmousewheel);

dojo.addOnLoad(function(){
	// get scrollIntoView for good measure
	// catch: dijit might not be loaded (yet?) so addonload
	if(window.dijit && window.dijit.scrollIntoView){
		dojo.connect(window.dijit,"scrollIntoView",function(node){
...
dojo.connect(dojo, "connect", 
	function(/*Widget*/ widget, /*String*/ event, /*Function*/ f){
		...
		// NOTE: Need mousewheel logic in events module

		if(widget && (!f || !f._mine)){
			var hitchedf = null;
			if(event.toLowerCase() == "onmousedown"){
				hitchedf = dojo.hitch(this,onmousedown);
			}else if(event.toLowerCase() == "onmousewheel" || event.toLowerCase() == 'dommousescroll'){
