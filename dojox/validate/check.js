...
dojox.validate.check = function(/*HTMLFormElement*/form, /*Object*/profile){
	...
	if(dojo.isArray(profile.uppercase)){
	...
	if(dojo.isArray(profile.lowercase)){
	...
	if(dojo.isArray(profile.ucfirst)){
		var fn = function(word) { return word.substring(0,1).toUpperCase() + word.substring(1).toLowerCase(); };
		for(i = 0; i < profile.ucfirst.length; i++){
			elem = form[profile.ucfirst[i]];
			if(_undef("type", elem) || elem.type != "text" && elem.type != "textarea" && elem.type != "password"){ continue; }
			elem.value = elem.value.replace(/\b\w+\b/g, fn);
		}
	}
	// Remove non digits characters from the input.
	if(dojo.isArray(profile.digit)){
	...
	if(dojo.isArray(profile.required)){
	...
	if(dojo.isArray(profile.required)){
		for (i = 0; i < profile.required.length; i++){
			...
			if(!dojo.isObject(profile.required[i]) || !profile.required[i]){ continue; }
			var numRequired;
			for(var name in profile.required[i]){ 
				if (dojo.isOwnProperty(profile.required[i], name)) {
					elem = form[name]; 
					numRequired = profile.required[i][name];
				}
			}
			// case 1: elem is a select box
			...
			// case 2: elem is a check box group
			else if(dojo.isArray(elem)){
				checked = 0;
				for(j = 0; j < elem.length; j++){
	...
	if(dojo.isObject(profile.constraints) && profile.constraints){
		// constraint properties are the names of fields to bevalidated
		for(name in profile.constraints){
			if (dojo.isOwnProperty(profile.constraints, name)) {
				elem = form[name];
				if(!elem) {continue;}
	...
	if(dojo.isObject(profile.confirm) && profile.confirm){
		for(name in profile.confirm){
			if (dojo.isOwnProperty(profile.confirm, name)) {
