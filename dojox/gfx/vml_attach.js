...
(function(){
	var attachFill = function(object){
		...
		var h, w, stops, t, i, fillStyle = null, r = object.rawNode, fo = r.fill;
		if(fo.on && fo.type == "gradient"){
			fillStyle = dojo.clone(dojox.gfx.defaultLinearGradient);
			...
			fillStyle = dojo.clone(dojox.gfx.defaultRadialGradient);
			...
			fillStyle = dojo.clone(dojox.gfx.defaultPattern);
	...
	var attachEllipse = function(object){
		...
		var style = object.rawNode.style,
			rx = parseInt(style.width, 10) / 2,
			ry = parseInt(style.height, 10) / 2;
		object.shape = dojox.gfx.makeParameters(dojox.gfx.defaultEllipse, {
			cx: parseInt(style.left, 10) + rx,
			cy: parseInt(style.top, 10) + ry,
			rx: rx,
			ry: ry
		});
	};
	...
	var attachCircle = function(object){
		...
		// object: dojox.gfx.Shape: an VML shape
		var style = object.rawNode.style, r = parseInt(style.width, 10) / 2;
		object.shape = dojox.gfx.makeParameters(dojox.gfx.defaultCircle, {
			cx: parseInt(style.left, 10) + r,
			cy: parseInt(style.top, 10)  + r,
			r:  r
		});
	};
	...
	var attachLine = function(object){
		...
		do{
			...
			shape.x1 = parseInt(p[1], 10);
			shape.y1 = parseInt(p[2], 10);
			shape.x2 = parseInt(p[4], 10);
			shape.y2 = parseInt(p[5], 10);
		}while(false);
	};
	...
	var attachText = function(object){
		...
		do{
			...
			switch(s["v-text-align"]){
				case "left":
					shape.x = parseInt(p[1], 10);
					shape.align = "start";
					break;
				case "center":
					shape.x = (parseInt(p[1], 10) + parseInt(p[4], 10)) / 2;
					shape.align = "middle";
					break;
				case "right":
					shape.x = parseInt(p[4], 10);
					shape.align = "end";
					break;
			}
			shape.y = parseInt(p[2], 10);
	...
	var attachPath = function(object){
		...
		var l = t.length;
		if(l >= 4 && !t[l - 1] && !t[l - 2] && !t[l - 3] && t[l - 4] == "l"){
			t.splice(l - 4, 4);
		}
		if(l){
			shape.path = t.join(" ");
	...
	dojox.gfx.attachNode = function(node){
		...
		switch(node.tagName.toLowerCase()){
			...
			case dojox.gfx.Image.nodeType:
				if(node.getAttribute("dojoGfxType") == "image"){
