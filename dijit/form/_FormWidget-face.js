dojo.provide("dijit.form._FormWidget-face");
...
dojo.declare("dijit.form._FormWidget", [dijit._Widget, dijit._Templated],
	...
	_setStateClass: function(){
		...
		for(var c in classHash){
			if (dojo.isOwnProperty(classHash, c)) {
				newClasses.push(c);
			}
		}
	...
	_handleOnChange: function(/*anything*/ newValue, /* Boolean? */ priorityChange){
		...
		if((this.intermediateChanges || priorityChange || priorityChange === undefined) &&
			((typeof newValue != typeof this._lastValueReported) ||
				this.compare(newValue, this._lastValueReported))){
	...
	destroy: function(){
		if(this._layoutHackHandle){
			dojo._getWin().clearTimeout(this._layoutHackHandle);
		}
		this.inherited(arguments);
	},
	...
	_layoutHack: function(){
		...
		console.warn('Skipping table resize workaround (FF2 issue)');
	}
});
...
dojo.declare("dijit.form._FormValueWidget", dijit.form._FormWidget,
{
	...
	_onKeyDown: (function() {
		var esc = dojo.keys.ESCAPE, fn, te, doc = dojo._getWin().document;
		
		if(dojo.isHostMethod(doc, 'createEvent')){ // ESCAPE needs help making it into keypress
			fn = function(e) {
				e.preventDefault();
				te = doc.createEvent('Events');
				te.initEvent('keypress', true, true);
				te.keyCode = esc;
				te.shiftKey = e.shiftKey;
				e.target.dispatchEvent(te);
			};
		} else if(dojo.isHostMethod(doc, 'createEventObject')){ 

			// NOTE: Object inferences (assumes IE)

			fn = function(e) {
				e.returnValue = false; // default behavior needs to be stopped here since keypress is too late
				te = doc.createEventObject();
				te.keyCode = esc;
				te.shiftKey = e.shiftKey;
