...
*/
if (jum_is_jsunit_hieatt) {
  /*
   * interface methods
   */
  jum.name = 'hieatt';
    // their debug() takes just two arguments (message, value). 
    // value is optional and is shown after a ': ' if present.
    // see jsUnitTracer.js
  jum.debug = debug;
  jum.info = inform;
  jum.warn = warn;

  // Hieatt assert* all throw JsUnitException.
  // Hieatt assertTrue and assertFalse throw an error if typeof argument is not boolean.
  jum.assertTrue = function() {assertTrue.apply(null, jum_arg_to_boolean(arguments))};
  jum.assertFalse = function() {assertFalse.apply(null, jum_arg_to_boolean(arguments))};
  jum.assertEquals = jum_wrap_assertEquals(assertEquals);

  jum.isFailureException = function(e) {return (e instanceof JsUnitException)};

  jum.areTestsChosen = function() {return typeof exposeTestFunctionNames != 'undefined'};

  jum.getChosenTestGroupName = function() {
     if (typeof top.jsUnitParmHash == 'undefined') return false;
     return typeof top.jsUnitParmHash['groupname'] == 'undefined' ? null : top.jsUnitParmHash['groupname'];
  };

  jum.setTests = function(alltests) {
    this.alltests_ = alltests;
    jum.setTestFunctionNames_(jum_get_all_function_names(alltests));
  }
   ...
   */
  jum.func_names_ = [];
  jum.setTestFunctionNames_ = function(funcnames) {
    //bu_alert("(jsunit_wrap.js) in setTestFunctionNames_, funcnames.length=" + funcnames.length);
    this.func_names_ = funcnames;
    var this_ = this;
    jum_global.exposeTestFunctionNames = function() {
       //alert("(jsunit_wrap.js) in exposeTestFunctionNames, this_=" + this_ + " funcnames.length=" + funcnames.length);
       return this_.func_names_;
    };
  }
...
/*
Jorg Schaible jsunit, http://jsunit.berlios.de

For basics, see jsunit/lib/JsUnit.js

If not otherwise specified, it expects 'AllTests' to be a instance of TestSuite and
have a method suite() to return a suite.

The base class 'TestSuite' looks for members of the prototype that are functions and have 
'test' anywhere in their name.
*/
else if (jum_is_jsunit_schaible) {
  /*
   * interface methods
   */
  jum.name = 'schaible';
  // their println takes just one argument.
  // Schaible doesn't have its own debug api, unlike Hieatt.
  jum.debug = function(line) {JsUtil_getSystemWriter().println(line)};
  jum.info = jum.debug;
  jum.warn = jum.debug;

  // We have to have an instance of an Assert object.
  // That is the only way to get an exception recorded as a "failures".
  // Any other exception is recorded as an "error".
  // These methods throw AssertionFailedError instances.
  var jum_assert = new Assert();

  // Schaible assertTrue and assertFalse already do our desired semantic of an eval.
  jum.assertTrue = function() {Assert_assertTrue.apply(jum_assert, arguments)};
  jum.assertFalse = function() {Assert_assertFalse.apply(jum_assert, arguments)};
  jum.assertEquals = jum_wrap_assertEquals(function() {Assert_assertEquals.apply(jum_assert, arguments)});

  jum.isFailureException = function(e) {return (e instanceof AssertionFailedError)};

  jum.areTestsChosen = function() {return typeof AllTests != 'undefined' && typeof AllTests.suite != 'undefined'};
  jum.getChosenTestGroupName = function() {return null};
  jum.setTests = function(alltests) {this.alltests_ = alltests; jum.make_AllTests_(alltests)};

  jum.runAll = function() {
    return TextTestRunner.prototype.main('--classic');
  };
  jum.runGroup = function(groupname) {
    return TextTestRunner.prototype.main('--classic' + ' Test' + groupname);
  };
  ...
  // internal method to create a TestSuite instance corresponding to the provided groupname.
  // This involves creating and adding TestCase instances for each test function.
  jum.make_TestSuite_ = function(groupname, alltests) {
    var testsuite_name = 'Test' + groupname;

    var testcase = function(name) {TestCase.call(this, name)};
    testcase.prototype = new TestCase();

    // attach test functions to prototype
    jum.attach_test_functions_(testcase, groupname, alltests);

    //var testsuite = new TestSuite(testsuite_name);
    var testsuite = function() {
       jum_debug("in testsuite constructor, groupname=" + groupname);
       TestSuite.call(this, testsuite_name);
       //this.addTest(testcase);
       this.addTestSuite(testcase);
    }
    testsuite.prototype = new TestSuite();
    testsuite.prototype.suite = function() {return new testsuite()};
    return testsuite;
  };

   // Unused. A failed attempt to eliminate the superfluous extra TestSuite layer
  jum.make_TestSuite2_ = function(groupname, alltests) {
    var testsuite_name = 'Test' + groupname;

    // new TestSuite(obj) only does introspection on obj if it is a function,
    // and in that case it gets its name by parsing obj.toString()
    var testsuite_obj;
    eval('testsuite_obj = function ' + testsuite_name + '(name){TestCase.call(this,name)}');

    jum_attach_test_functions(testsuite_obj, groupname, alltests);

    var testsuite = function() {};
    testsuite.prototype.suite = function() {return new TestSuite(testsuite_obj)};
    return testsuite;
  };

  // internal method to add some methods to a TestCase instance's prototype, so
  // that JsUnit reflection will find it.
  jum.attach_test_functions_ = function(testcase, groupname, alltests) {
    var groupdef = jum_get_groupdef(alltests, groupname);
    ...
    for (var i=0;i<testdefs.length; ++i) {
       ...
       jum_debug("attaching test for group '" + groupname + "', testname " + testname);
       // prepend 'test' so reflection finds it.
       var testcase_funcname = 'test_' + testname;
       testcase.prototype[testcase_funcname] = func;
    }
  }

  // make a wrapper TestSuite for the individual TestSuite instances for each group.
  jum.make_AllTests_ = function(alltests) {
    jum_global.AllTests = function() {TestSuite.call( this, "AllTests" )};

    // yes, AllTests.suite() returns another instance of AllTests().
    // oh, and BaseTestRunner_getTest calls <code>new AllTests.prototype.suite()</code>.
    var AllTests_suite = function() {
      var allsuite = new AllTests();

      var groupnames = jum_get_groupnames(alltests);
      for (var i=0;i<groupnames.length;++i) {
        var groupname = groupnames[i];
        var testsuite = jum.make_TestSuite_(groupname, alltests);
        // same as suite.addTest (new TestSuite (testsuite));
        // TestSuite(obj) will introspect it for member functions
        //allsuite.addTestSuite(testsuite);
        allsuite.addTest(testsuite.prototype.suite());
      }
      return allsuite;
    };
...
/*
Our own builtin JsUnit implementation.
*/
else {
