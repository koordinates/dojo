...
function test_validate_isValidLuhn(){
	jum.assertTrue("test1", dojo.validate.isValidLuhn('5105105105105100')); //test string input
	jum.assertTrue("test2", dojo.validate.isValidLuhn('5105-1051 0510-5100')); //test string input with dashes and spaces (commonly used when entering card #'s)
	jum.assertTrue("test3", dojo.validate.isValidLuhn(38520000023237)); //test numerical input as well
	jum.assertFalse("test4", dojo.validate.isValidLuhn(3852000002323)); //testing failures
	jum.assertTrue("test5", dojo.validate.isValidLuhn(18)); //length doesnt matter
	jum.assertFalse("test6", dojo.validate.isValidLuhn(818181)); //short length failure
}

function test_validate_isValidCvv(){
	jum.assertTrue("test1", dojo.validate.isValidCvv('123','mc')); //string is ok
	jum.assertFalse("test2", dojo.validate.isValidCvv('5AA','ec')); //invalid characters are not ok
	jum.assertTrue("test3", dojo.validate.isValidCvv(723,'mc')); //numbers are ok too
	jum.assertFalse("test4", dojo.validate.isValidCvv(7234,'mc')); //too long
	jum.assertTrue("test5", dojo.validate.isValidCvv(612,'ec'));
	jum.assertTrue("test6", dojo.validate.isValidCvv(421,'vi'));
	jum.assertTrue("test7", dojo.validate.isValidCvv(543,'di'));
	jum.assertTrue("test8", dojo.validate.isValidCvv('1234','ax')); 
	jum.assertTrue("test9", dojo.validate.isValidCvv(4321,'ax'));
	jum.assertFalse("test10", dojo.validate.isValidCvv(43215,'ax')); //too long
	jum.assertFalse("test11", dojo.validate.isValidCvv(215,'ax')); //too short
}
