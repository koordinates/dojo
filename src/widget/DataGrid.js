...
dojo.widget.defineWidget("dojo.widget.DataGrid",dojo.widget.HtmlWidget,{
	...
	dataCellElts: [], //holds reference of the cells that contain data
...
//		this.binders = [];
	...
	_drawBody: function(){
		...
		for(var i = 0; i < rowsToRender; i++){
			...
			for(var j = 0; j < colsToRender; j++){
				...
				this.dataCellElts.push(cell); //store reference (handy for updates and paging)			
				tr.appendChild(cell);
			}
	...
	_updateBody: function(){
		...
		for(var i=0; i<rowsToRender; i++){
			...
			for(var j=0; j<colsToRender; j++){
				var cell = this.dataCellElts[ j + (i*colsToRender) ];
...
//TODO: Replace with unique id util			
		...
		this.tableData = model.getTableController(); 	
