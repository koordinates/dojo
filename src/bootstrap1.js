...
function dj_debug() {
    var args = arguments;
/*@end @*/
    if (typeof dojo.hostenv.println != 'function') dj_throw("attempt to call dj_debug when there is no dojo.hostenv println implementation (yet?)");
...
function dj_throw(message) {
    if (typeof dojo.hostenv != 'undefined' && typeof dojo.hostenv.println != 'undefined') dojo.hostenv.println("fatal error: " + message);
    throw Error(message);
}
...
function dj_inherits(subclass, superclass) {
 if (typeof superclass != 'function') { dj_throw("eek: superclass not a function: " + superclass +"\nsubclass is: " + subclass);}
...
dojo.hostenv.getBaseScriptUri = function() {
	...
	if (!uri) dj_throw("Nothing returned by getLibraryScriptUri(): " + uri);
    }
    var lastslash = uri.lastIndexOf('/');
    var base = this.base_relative_path_ + (lastslash == -1 ? '' : uri.substring(0,lastslash + 1)); // inclusive of slash
...
dojo.hostenv.loadPath = function(relpath) {
    if (!relpath) dj_throw("Missing relpath argument");
    if (relpath.charAt(0) == '/' || relpath.match(/^\w+:/)) dj_throw("Illegal argument '" + relpath + "'; must be relative path");
...
dojo.hostenv.loadModule = function(modulename, exact_only) {
    ...
    if (typeof this.loading_modules_[modulename] !== 'undefined') dj_throw("recursive attempt to load module '" + modulename + "'");
    ...
    if (!module) dj_throw("Module symbol '" + modulename + "' is not defined after loading '" + relpath + "'"); 

    return module;
}

function dj_load(modulename, exact_only) {return dojo.hostenv.loadModule(modulename, exact_only);}
...
dojo.hostenv.findModule = function(modulename, must_exist) {
    // check cache
    if (typeof this.modules_[modulename] != 'undefined') return this.modules_[modulename];

    // see if symbol is defined anyway
    var module = dj_eval_object_path(modulename);
    if (typeof module !== 'undefined' && module) return this.modules_[modulename] = module;

    if (must_exist) dj_throw("no loaded module named '" + modulename + "'");
    return null;
}
